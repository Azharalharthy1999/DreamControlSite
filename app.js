...
let remCounter = 0;
const REM_REQUIRED = 15; // Highest for no blue on gentle/lift
let calmCounter = 0;
const CALM_RESET = 3; // Instant reset to green
let varianceBuffer = [];
const VARIANCE_BUFFER_SIZE = 20; // Strong noise filter
let calmVarianceAvg = localStorage.getItem('calmVarianceAvg') ? parseFloat(localStorage.getItem('calmVarianceAvg')) : 8;
let remSpikeThreshold = localStorage.getItem('remSpikeThreshold') ? parseFloat(localStorage.getItem('remSpikeThreshold')) : 1.5;
let remGyroThreshold = localStorage.getItem('remGyroThreshold') ? parseFloat(localStorage.getItem('remGyroThreshold')) : 18;
let baroBuffer = [];
