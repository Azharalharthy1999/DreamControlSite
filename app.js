...
let remCounter = 0;
const REM_REQUIRED = 12; // Higher for no blue on lift
let calmCounter = 0;
const CALM_RESET = 5; // Short for fast reset to green
let varianceBuffer = [];
const VARIANCE_BUFFER_SIZE = 12; // Longer filter for noise
let calmVarianceAvg = localStorage.getItem('calmVarianceAvg') ? parseFloat(localStorage.getItem('calmVarianceAvg')) : 7; // Raised for stable green
let remSpikeThreshold = localStorage.getItem('remSpikeThreshold') ? parseFloat(localStorage.getItem('remSpikeThreshold')) : 1.0; // Raised to ignore small wobbles
let remGyroThreshold = localStorage.getItem('remGyroThreshold') ? parseFloat(localStorage.getItem('remGyroThreshold')) : 12;
let baroBuffer = [];
